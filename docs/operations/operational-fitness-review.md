...
![Uma visão geral do processo de revisão de adequação operacional](_images/ofr-flow.png)
...
1. **Analisar dependências de serviços**. A maioria das operações de negócios requer orquestração entre vários serviços de suporte. É importante entender as dependências entre os serviços e o fluxo de transações de missão crítica por meio desses serviços.
    ...
    Considere também as dependências entre os serviços locais e os serviços do Azure. No exemplo de compra-carrinho, o serviço de gerenciamento de estoque de estoque pode ser hospedado localmente e ingerir dados inseridos por funcionários de um depósito físico. No entanto, ele pode armazenar dados fora do local em um serviço do Azure, como o [armazenamento do Azure](/azure/storage/common/storage-introduction), ou um banco de dado, como [Azure Cosmos DB](/azure/cosmos-db/introduction).
